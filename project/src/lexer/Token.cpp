...
namespace tinyc::lexer {
	...
	Token::Token(TokenType type, std::string lexeme, SourceLocation location)
			: type(type), lexeme(std::move(lexeme)), location(std::move(location)), hasValue(false) {
	}

	Token::Token(TokenType type, int value, std::string lexeme, SourceLocation location)
			: type(type), lexeme(std::move(lexeme)), location(std::move(location)), intValue(value), hasValue(true) {
		if (type != TokenType::INTEGER_LITERAL) {
			throw std::invalid_argument("Cannot create non-integer token with integer value");
		}
	}

	Token::Token(TokenType type, double value, std::string lexeme, SourceLocation location)
			: type(type), lexeme(std::move(lexeme)), location(std::move(location)), doubleValue(value), hasValue(true) {
		if (type != TokenType::DOUBLE_LITERAL) {
			throw std::invalid_argument("Cannot create non-double token with double value");
		}
	}

	Token::Token(TokenType type, char value, std::string lexeme, SourceLocation location)
			: type(type), lexeme(std::move(lexeme)), location(std::move(location)), charValue(value), hasValue(true) {
